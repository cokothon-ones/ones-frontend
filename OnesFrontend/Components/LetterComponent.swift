...
struct LetterComponent: View {
    var mode: LetterMode = .edit

    @State var showLetterModal: Bool = false
    ...
    var body: some View {
            ...
            HStack {
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: 999)
                        .inset(by: -0.5)
                        .stroke(mode == .read ? Color.white : Color.black, lineWidth: 1)
                )
                .foregroundStyle(mode == .read ? Color.white : Color.black)

                Spacer()

                Image(mode == .read ? "chevron_right" : "mode_edit_outline")
        ...
        // 나의 것이 아닌 경우에는 블러 처리
        .blur(radius: mode == .read ? 0 : 5)
        .padding(16)
        .frame(width: 256, height: 168, alignment: .topLeading)
        .background(
            mode == .read ?
                Color(red: 0.37, green: 0.48, blue: 0.68)
                :
